...
class CircleAnimation {
    ...
    createCircles() {
        ...
        for (let i = 0; i < this.NRPTS; i++) {
            ...
            this.lastPositions.push({x: circle.x, y: circle.y, dx: circle.dx, dy: circle.dy, length:circle.length, speed:circle.speed}); // saves its initial position in this.lastPositions
        }
    ...
    resetAnimation() {
        ...
        this.circles.forEach((c) => {
            if (c.interval) {
                clearInterval(c.interval); // clears interval
                c.interval = null; // Clear interval reference
            }
        });
        ...
        this.circles.forEach((c, index) => {
            ...
            c.length = this.lastPositions[index].length;
            c.speed = this.lastPositions[index].speed;
            c.steps = 0;
        });
    ...
    updateLastPositions() {
        this.circles.forEach((c, index) => {
            this.lastPositions[index] = {x: c.x, y: c.y, dx: c.dx, dy: c.dy, length: c.length, speed: c.speed};
        });
    }
